...
<div class="grid-item" :style="`grid-column: span ${width}`">
  <div class="grid-item-tags kimera-text-filter-tags">
    <span v-for="(tag, index) in project.tags">{{tag}}{{index < project.tags.length -1 ? ', ': ''}} </span>
  </div>
  <div class="grid-item-inner center-all relative kimera-text-kacheln" :class="checkProjectType" :title="project.title">
