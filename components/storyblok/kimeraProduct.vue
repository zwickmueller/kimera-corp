<template>
  <div v-editable="blok" class="kimera-product">
    ...
    <div
      ref="sticky"
      class="product-description-wrapper bottom-gradient-mobile"
    >
      ...
      <div class="product-description">
        <component
          v-for="blok in blok.productBody"
          :key="blok._uid"
          :blok="blok"
          :is="blok.component"
        />
...
export default {
  ...
  methods: {
    handleScroll() {
      ...
      let topContainerDomHeight = document
        .querySelector(".project div")
        .getBoundingClientRect().height;
      let diff = window.scrollY - scrollDom.getBoundingClientRect().height;

      stickyDom.style.height = window.innerHeight - diff + "px";
      stickyDom.style.maxHeight =
        clamp(
          window.scrollY + (window.innerHeight - topContainerDomHeight),
          0,
          window.innerHeight
        ) + "px";
    },
  ...
  .product-description-wrapper {
    ...
    @include until($tablet) {
      ...
      padding: 0 var(--kimera-side-padding)
        calc(var(--kimera-side-padding) * 2.5) var(--kimera-side-padding);
    }
    // height: 100%;
    max-height: 100vh;
    display: flex;
