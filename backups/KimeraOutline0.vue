...
<aside v-if="shouldRender" ref="header" @mouseenter="mouseEnter" @mouseleave="mouseLeave">
  ...
  <div class="kimera-text-filter kimera-outline" v-if="!mouseEntered" :key="0" data-index="0">
  ...
  <div class="kimera-text-filter kimera-outline" v-else :key="1" data-index="1">
    ...
    <button @click="handleNavigation(headline.domId)" class="kimera-outline-button kimera-button button-reset" v-for="(headline, i) in headlines" :key="headline.text" :data-order-index="i" :style="`order: ${i +1}`">
